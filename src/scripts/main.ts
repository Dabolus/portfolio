...
window.addEventListener('load', () => {
  ...
  const fragments: NodeListOf<HTMLElement> = document.querySelectorAll(
    '#page-container > .page',
  );
  ...
  const homeLinks: HTMLElement[] = [
    ...Array.prototype.slice.call(menuLinks),
    since2004Link,
  ];
  const homeSections: NodeListOf<HTMLElement> = document.querySelectorAll(
    '#i-am, #menu',
  );
  ...
  since2004Link.addEventListener('focus', () => smoothScroll(0));
  ...
  dataPromise
    ...
    .catch(
      e =>
        (document.body.innerHTML = `<h1>Unexpected fatal error!</h1><pre>${e}</pre>`),
    );
  ...
  installRouter(({ pathname }) => {
    ...
    if (path === '') {
      ...
      main.className = '';
      homeLinks.forEach(link => link.setAttribute('tabindex', '0'));
      homeSections.forEach(section => section.removeAttribute('aria-hidden'));
      const oldElement: HTMLElement = document.querySelector(
        `#page-container > .page[data-page="${oldPath}"`,
      );
      if (oldElement) {
        oldElement.focus();
      }
      ...
      setTimeout(() => {
        ...
        fragments.forEach(
          fragment =>
            (fragment.className =
              fragment.dataset.page === path ? 'page active' : 'page'),
        );
      ...
      setTimeout(() => {
        homeLinks.forEach(link => link.setAttribute('tabindex', '-1'));
