...
export const ssr = functions.https.onRequest(async ({ path = '/' }, res) => {
  ...
  res.setHeader(
    ...
    "script-src 'self' 'unsafe-inline' https://www.google-analytics.com https://www.google.com/recaptcha/ https://www.gstatic.com/recaptcha/; style-src 'self' 'unsafe-inline'; img-src 'self' 'unsafe-inline' data: https://firebasestorage.googleapis.com https://www.google-analytics.com; frame-src 'self' https://giorgio.garasto.blog https://www.google.com/recaptcha/; default-src 'self'",
  );
  res.send(
